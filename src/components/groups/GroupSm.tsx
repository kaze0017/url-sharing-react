...
export default function GroupSm({ group, selected }: GroupSmProps) {
  ...
  function toggle(group: GroupType) {
    // if (selectedGroups.some((item: GroupType) => item.group_id === group.group_id)) {
    //   dispatch(
    //     setSelectedGroups(
    //       selectedGroups.filter((item:GroupType) => item.group_id !== group.group_id)
    //     )
    //   );
    //   dispatch(setGroupsToDisplay([...selectedGroups, group]));
    // } else {
    //   dispatch(setSelectedGroups([...selectedGroups, group]));
    //   dispatch(
    //     setGroupsToDisplay(
    //       selectedGroups.filter((item) => item.group_id !== group.group_id)
    //     )
    //   );
    // }
  }
